...
function DataFetching(props) {
  ...
  function useFetch(url, defaultResponse) {
      ...
      useEffect(() => {
          if(values.length === 0)
        getDataFromAPI();
        const filteredUser = values.filter(person =>
          person.name.first.toLowerCase().includes(searchUser.toLowerCase()) | person.name.last.toLowerCase().includes(searchUser.toLowerCase()) 
          );
...
return (
    <UserContext.Provider value={[values,setValues,searchResults,setSearchResults,reset,setReset]}>
      <div className="App">
        <div className ="users">  
          <div className="user__searchInput"> 
              ...
              <button onClick={ handle_SortName_Descending }>Name Decending</button>
              </span>
          </div>
          </div>
          <div>
          <div>
              ...
              {searchResults.map(
                  ...
                  item => (
                      ...
                      className="details">
                          ...
                          <FontAwesomeIcon icon={faTrashAlt}  onClick={() => handleRemove(searchResults.indexOf(item))}/>
                      </div>
              ...
              <h2>Add new user</h2>
               <form >
                  ...
                  <botton type="botton"  onClick={handleSubmit}>ADD</botton> 
              </div>
        </div>
      </div>
